...
<section class="mx-auto w-full max-w-3xl space-y-3">
  <header class="space-y-3">
    <h1 class="text-base-content text-3xl font-bold">Nueva palabra</h1>
    <p class="text-base-content/70">
      Comparte palabras del español barranquillero y nosotros nos encargamos de revisarlas antes de
      publicarlas.
    </p>
    <div
      class="alert bg-base-100 border-base-200 text-base-content/80 flex flex-col border text-sm font-semibold"
    >
      <div class="flex w-full items-center justify-start space-x-2">
        <AlertCircleIcon class="text-primary size-5 shrink-0" aria-hidden="true" />
        <h3>
          Antes de enviar, revisa las <a class="link link-primary" href="/guidelines">
            pautas de contenido
          </a>.
        </h3>
      </div>
      <p class="text-left text-xs">
        En resumen: Comparte definiciones que otras personas puedan entender y nunca publiques
        discurso de odio ni información personal.
      </p>
    </div>
  </header>
  ...
  <form
    class="card bg-base-100 space-y-6 p-6 shadow-lg"
    onsubmit={(event) => {
        ...
        {#snippet children(field)}
          <div class="form-control gap-2">
            ...
            {#if field.state.meta.errors?.length && (field.state.meta.isTouched || formState.current.isSubmitted)}
              <p class="text-error text-sm">{field.state.meta.errors[0]?.message}</p>
            {/if}
          </div>
        {/snippet}
      </Field>
