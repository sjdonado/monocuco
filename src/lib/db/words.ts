...
export const runWordsMigration = async (
  ...
  // Stage 1: Fetch parquet (SW will serve from cache and invalidate via data.parquet.json)
  ...
  const response = await fetch(`/${DATA_FILE_NAME}`, { cache: 'no-cache' });
  if (!response.ok || !response.body) {
    const buf = new Uint8Array(await response.arrayBuffer());
    await db.registerFileBuffer(DATA_FILE_NAME, buf);
  } else {
    // Stream with progress (if content-length present)
    ...
    while (true) {
      ...
      const pct = total > 0 ? (received / total) * 40 : 10;
      splashScreenProgress.set({
        isRunning: true,
        stage: 'downloading',
        percentage: pct,
        message: `Descargando datos... ${Math.round(pct)}%`,
        downloadProgress: { loaded: received, total }
      });
    }
